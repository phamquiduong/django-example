...
def register(email, password) -> dict[str, Any]:
    ...
    try:
        ...
        raise BadRequestException(api_code=APICode.REGISTER, error_code=112, error_detail='Password is not valid',
                                  error_fields={'password': exc.messages}) from exc

    if User.objects.filter(email=email).exists():
        raise ConflictException(api_code=APICode.REGISTER, error_code=101, error_detail='User already registered')
